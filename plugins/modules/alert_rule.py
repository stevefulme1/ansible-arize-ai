...
"""Ansible module for managing Arize AI alert rule."""
...
---
module: alert_rule
short_description: Manage alert rules in Arize AI
description:
    - Manage alert rules in Arize AI via the Arize AI REST API.
version_added: "1.0.0"
author:
...
- name: Create alert rule
  stevefulme1.arize_ai.alert_rule:
    name: "example-value"
...
RETURN = r"""
alert_rule:
    description: Details of the alert rule.
    returned: On success.
    type: dict
"""
...
def get_resource(module, api_url, headers):
    ...
    if not resource_id:
        return None
    try:
        url = f"{api_url}/api/v1/alert_rule/{resource_id}"
...
def update_resource(module, api_url, headers, existing):
    ...
    response = requests.put(
        f"{api_url}/api/v1/alert_rule/{resource_id}",
        headers=headers, json=payload,
        verify=module.params["validate_certs"],
        timeout=30,
    )
...
def needs_update(params, existing):
    ...
    updatable = ['name', 'monitor_id', 'threshold']
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, alert_rule=resource)
    ...
    module.exit_json(changed=False, alert_rule=existing)
