...
"""Ansible module for managing Arize AI model."""
...
---
module: model
short_description: Manage models in Arize AI
description:
    - Manage models in Arize AI via the Arize AI REST API.
version_added: "1.0.0"
author:
...
def get_resource(module, api_url, headers):
    ...
    if not resource_id:
        return None
    try:
        url = f"{api_url}/v2/models/{resource_id}"
...
def update_resource(module, api_url, headers, existing):
    ...
    response = requests.put(
        f"{api_url}/v2/models/{resource_id}",
        headers=headers, json=payload,
        verify=module.params["validate_certs"],
        timeout=30,
    )
