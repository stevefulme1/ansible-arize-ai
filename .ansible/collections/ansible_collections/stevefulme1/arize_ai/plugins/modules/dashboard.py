...
"""Ansible module for managing Arize AI dashboard."""
...
---
module: dashboard
short_description: Manage dashboards in Arize AI
description:
    - Manage dashboards in Arize AI via the Arize AI REST API.
version_added: "1.0.0"
author:
...
options:
    ...
    space_id:
        description:
            - The space id parameter.
        type: str
        required: true
    dashboard_id:
        description:
            - The dashboard id parameter.
        type: str
        required: false
    state:
        description:
...
- name: Create dashboard
  stevefulme1.arize_ai.dashboard:
    name: "example-value"
    space_id: "example-value"
    state: present

- name: Delete dashboard
  stevefulme1.arize_ai.dashboard:
    dashboard_id: "example-value"
    state: absent
"""
...
def get_resource(module, api_url, headers):
    ...
    if not resource_id:
        return None
    try:
        url = f"{api_url}/api/v1/dashboard/{resource_id}"
...
def create_resource(module, api_url, headers):
    ...
    if module.params.get("space_id"):
        payload["space_id"] = module.params["space_id"]
    response = requests.post(
        f"{api_url}/api/v1/dashboard",
...
def update_resource(module, api_url, headers, existing):
    ...
    response = requests.put(
        f"{api_url}/api/v1/dashboard/{resource_id}",
        headers=headers, json=payload,
        verify=module.params["validate_certs"],
        timeout=30,
    )
...
def main():
    ...
    if existing is None:
        ...
        module.exit_json(changed=True, dashboard=resource)
